#ifndef ISL_INTERFACE_PLAIN_csharp_H
#define ISL_INTERFACE_PLAIN_csharp_H
 ...
 */
struct checked_csharp_type_printer : public csharp_type_printer {
 ...
 */
class plain_csharp_generator : public csharp_generator {
...
public:
  plain_csharp_generator(SourceManager &SM, set<RecordDecl *> &exported_types,
...
private:
  ...
  void print_interops(ostream &os);
  void print_class(ostream &os, const isl_class &clazz);
  void print_class_interops(ostream &os, const isl_class &clazz);
  void print_method_interop_type(ostream &os,FunctionDecl *fd);
  void print_interop_restype(ostream &os,FunctionDecl *fd);
  void print_interop_argtypes(ostream &os,FunctionDecl *fd);
  ...
  std::unique_ptr<csharp_type_printer> type_printer();
  std::string get_return_type(const CSharpMethod &method);
  string generate_callback_args(QualType type, bool csharp, bool with_arg);
  string generate_callback_type(QualType type, bool csharp);
  string isl_bool2csharp();
  string isl_namespace();
  string param2csharp(QualType type);
};
 ...
 */
struct plain_csharp_generator::plain_printer
    : public csharp_generator::class_printer {
  plain_csharp_generator &generator;

  plain_printer(std::ostream &os, const isl_class &clazz,
                plain_csharp_generator &generator, bool is_declaration)
  ...
  void print_full_method_header(const CSharpMethod &method);
 ...
 */
struct plain_csharp_generator::decl_printer
    : public plain_csharp_generator::plain_printer {
  ...
  virtual void print_method(const CSharpMethod &method) override;
  virtual void print_method(const CSharpConversionMethod &method) override;
 ...
 */
struct plain_csharp_generator::impl_printer
    : public plain_csharp_generator::plain_printer {
  ...
  virtual void print_ctx() override;
  virtual void print_method_separator() override;
  void print_set_persistent_callback(const CSharpMethod &method);
  virtual void print_persistent_callbacks() override;
  void print_argument_validity_check(const CSharpMethod &method);
  void print_save_ctx(const std::string &ctx);
  void print_save_ctx(const CSharpMethod &method);
  void print_on_error_continue();
  void print_exceptional_execution_check(const CSharpMethod &method);
  void print_method_return(const CSharpMethod &method);
